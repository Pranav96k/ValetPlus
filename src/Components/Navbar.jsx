...
const Navbar = () => {
  ...
  const id = pathSegments.length > 1 ? pathSegments[1] : pathSegments[0]; // Adjust based on your path structure

  console.log(id);

  return (
    <nav className=" w-full flex max-w-[1519px] mx-auto bg-[#0C79B6] justify-between p-3 shadow-md items-center ">
      {/* Logo */}
      <div className="flex gap-2 items-center flex-1 sm:mx-4 md:mx-6 lg:mx-8 ">
        <span className="font-medium text-2xl text-white lg:text-4xl cursor-pointer">
          ValetPlus+
        </span>
      </div>
      ...
      <div className="flex gap-4 items-center justify-end sm:mx-4 md:mx-6 lg:mx-8">
        <Link to="/home">
          {/* Home Button */}
          {id !== "login" && id !== "home" && (
            <span className="mx-5 font-serif text-white cursor-pointer">
              Home
            </span>
          )}
        </Link>

        <Link to="/customeronboarding">
          {id === "home" && (
            <button className="items-center border border-bl px-2 py-2 bg-white rounded-lg hover:border-grey-600">
